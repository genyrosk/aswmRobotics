...
XMLError XMLAttribute::QueryLongValue( long* value ) const
{
...
void XMLElement::SetText( long v )
{
    char buf[BUF_SIZE];
    XMLUtil::ToStr(v, buf, BUF_SIZE);
    SetText(buf);
...
XMLError XMLElement::QueryLongText( long* lval ) const
{
    if ( FirstChild() && FirstChild()->ToText() ) {
        const char* t = FirstChild()->Value();
        if ( XMLUtil::ToLong( t, lval ) ) {
            return XML_SUCCESS;
        }
        return XML_CAN_NOT_CONVERT_TEXT;
    }
